...
async def _main_stream(
    ...
    if _is_followup:
        ...
        _fup_instruction = (
            ...
            "3. PLANNING OFFER (1 sentence, must end with ?) — the planning offer is MANDATORY. "
            "Never close with a preference question alone.\n"
            "   • SHAPE A — if ONE destination is clear from context: one sentence that both invites "
            "more preferences and offers to start planning, NAMING the destination. Shape (never copy "
            "the wording): 'Would you like to share more about what you're after, or shall we start "
            "planning your trip to [Destination]?'\n"
            "   • SHAPE B — if multiple destinations are still on the table: one sentence naming 2-3 of "
            "them and asking which they'd like to plan a trip to, leaving room to keep exploring. Shape "
            "(never copy the wording): 'Which of these would you like to plan a trip to — or would you "
            "like to keep exploring to find the right fit?'\n"
            "   ⚠️ A CONTINENT or BROAD REGION (Europe, Southeast Asia, Central America, the Caribbean) "
            "is NEVER one destination — never offer to 'start planning your Europe trip'. If the place "
            "you would name still leaves 'but where exactly?' unanswered, use SHAPE B.\n"
            "   Write it fresh in your own words every time. ONE sentence with two branches — never two "
            "separate questions. The plan-vs-explore branch is REQUIRED and is NOT an 'option list' "
            "violation. Always ends with '?'.\n\n"
